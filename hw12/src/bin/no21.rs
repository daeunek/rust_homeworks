...
enum Text {
    Plain(String),
    Repeated(Box<Text>, usize), //new variant for repeated text
...
impl Text{
    fn value(&self) -> String {
        match self{
            ...
            Text::Repeated(subtxt, count) => subtxt.value().repeat(*count),
            Text::Joined(subtxts, seperator) => {
                let mut result = String::new();
                for (i, subtxt) in subtxts.iter().enumerate(){
                    result.push_str(&subtxt.value());
                    if i < subtxts.len() - 1{
                        result.push_str(&seperator.value());
                    }                
                }
                result
            }
